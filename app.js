...
app.get("/articles/:article_ID", function(req, res) {
  ...
  db.all(
    "select a.article_ID, a.title, a.content, a.username, u.fname || ' ' || u.lname as 'fullName' from article a, user u where a.article_ID = ? and a.username = u.username",
    [articleID],
    function(err, rows) {
      var article = rows[0];
      var data = {
...
app.get("/blog", function(req, res) {
  ...
  db.all(
    "SELECT a.title, a.article_ID, substr(a.content, 1, 500) || '...' as 'articleContent', u.fname || ' ' || u.lname as 'fullName' FROM user u, article a WHERE a.username = u.username",
    function(err, rows) {
      var articles = rows;
      var data = {
        bgPicture: "blog-bg",
...
app.get("/edit/:article_ID", function(req, res) {
  if (req.isAuthenticated()) {
    ...
    var articleID = req.params.article_ID;
    db.all(
      "SELECT title, content FROM article WHERE article_ID = ?",
      [articleID],
      function(err, rows) {
...
app.post("/editArticle", function(req, res) {
  ...
  form.parse(req, function(err, fields) {
    ...
    db.run(
      "UPDATE article SET title = ?, content = ?, username = ? WHERE article_ID = ?",
      [title, content, username, articleID],
      function(err) {
